...
│   ├── Application/                 # App logic + DSP
│   │   ├── inc/                     # Public headers (e.g. DCTCalculator.hpp)
│   │   └── src/                     # Implementations
│   ├── Drivers/                     # Hardware drivers (BGT60TRXX, BMI270, PWM)
│   │   ├── inc/
│   │   └── src/
│   ├── MachineLearning/
│   │   └── GestureDetection/        # Gesture model/code
│   ├── MessageSystem/               # Publish/subscribe message bus
│   │   ├── inc/                     # MessageSender, Node, Communication.hpp
│   │   └── src/
│   ├── RTOS/                        # FreeRTOS helpers + threads
│   │   ├── inc/
│   │   ├── src/
│   │   └── Threads/
│   │       ├── inc/                 # Thread<T>, Radar/Motion/USB…
│   │       └── src/
│   └── Wrappers/                    # HAL wrappers (SPI, Mutex, GPIO)
│       ├── inc/
│       └── src/
│
├── Test/                            # Unit tests (Meson + GTest)
│   ├── main.cpp
│   ├── MessageTest/
│   ├── MessageExchangeTest/
│   ├── ThreadsTest/
│   ├── Mocks/
│   │   ├── inc/                     # GPIOMock, TaskMock, SPIMock, …
│   │   └── src/
│   └── Stubs/
│       ├── inc/                     # FreeRTOS/HAL stubs (headers)
│       └── src/                     # Stub sources for tests
│
├── bsps/                            # Board Support Package (CY8CKIT-062S2-AI)
│   └── TARGET_APP_CY8CKIT-062S2-AI/
│       ├── config/                  # design.modus, GeneratedSource
│       ├── COMPONENT_CM0P/          # CM0+ startup
│       ├── COMPONENT_CM4/           # CM4 startup
│       └── deps/                    # BSP deps
│
├── deps/                            # ModusToolbox dependencies (.mtb)
│   ├── *.mtb
│   └── libs/                        # Resolved sources (make getlibs)
│
├── build/                           # Build artifacts
│   ├── compile_commands.json
│   └── APP_CY8CKIT-062S2-AI/Debug/
│
├── main.c                           # RTOS entry
├── Makefile                         # ModusToolbox build
├── meson.build                      # Tests build (Meson)
├── openocd.tcl                      # Debug script
└── .vscode/                         # Generated by `make vscode` (not committed)
```
